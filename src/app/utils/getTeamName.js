export const getTeamName = (index, balancedTeams, parsedCustomColours, customColourInput) => {
    ...
    if (parsedCustomColours.length > 0 && parsedCustomColours[index]) {
      ...
      const colourNames = customColourInput.split(',').map(c => c.trim()).filter(c => c.length > 0);
